package net.stickia.carpethex.casting.patterns.spells
...
import net.stickia.carpethex.networking.SetLookPitchS2CMsg
...
class OpCongrats : SpellAction {
    ...
    override fun execute(args: List<Iota>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>> {
        ...
        if (target !is ServerPlayer) throw MishapBadEntity(
            target,
            Component.translatable("text.carpethex.congrats.player")
        )
    ...
    private data class Spell(val player: ServerPlayer) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            player.sendSystemMessage(Component.translatable("text.carpethex.congrats", player.displayName));
            CarpetHexNetworking.sendToPlayer(player, SetLookPitchS2CMsg(-90f))
        }
    }
}
